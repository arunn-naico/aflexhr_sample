...
foreach ($_POST as $key => $value) { if (is_string($value)) { $_POST[$key] = kleor_quotes_entities($_POST[$key]); } }
$GLOBALS['selection_criteria'] = ''; $selection_criteria = '';
$all_tables_keys = contact_manager_all_tables_keys($tables);
...
function contact_manager_no_items($table) {
switch ($table) {
...
function contact_manager_row_actions($table, $item) {
global $wpdb;
switch ($table) {
case 'forms':
...
function contact_manager_single_page_slug($table) {
switch ($table) {
...
function contact_manager_table_name($table) {
global $wpdb;
return $wpdb->prefix.'contact_manager_'.$table; }


function contact_manager_table_undisplayed_keys($tables, $table, $back_office_options) {
global $wpdb;
$undisplayed_modules = (array) $back_office_options[contact_manager_single_page_slug($table).'_page_undisplayed_modules'];
...
function contact_manager_table_data($table, $column, $item) {
switch ($table) {
...
$custom_fields = (array) $back_office_options[contact_manager_single_page_slug($table).'_page_custom_fields'];
$item_custom_fields = (array) unserialize($data);
...
if ($description != '') { $description = ' <span class="description">('.$description.')</span>'; } $table_td = $data.$description;
if ((function_exists('commerce_data')) && (current_user_can('view_commerce_manager')) && ($data > 0)) { $table_td .= ' <span class="row-actions edit"><a href="admin.php?page=commerce-manager-client-category&amp;id='.$data.'">'.__('Edit', 'contact-manager').'</a></span>'; } break;
...
if ($description != '') { $description = ' <span class="description">('.$description.')</span>'; } $table_td = $data.$description;
if ((function_exists('membership_data')) && (current_user_can('view_membership_manager')) && ($data > 0)) { $table_td .= ' <span class="row-actions edit"><a href="admin.php?page=membership-manager-member-category&amp;id='.$data.'">'.__('Edit', 'contact-manager').'</a></span>'; } break;
...
if ($description != '') { $description = ' <span class="description">('.$description.')</span>'; } $table_td = $data.$description;
if ((function_exists('member_area_data')) && (current_user_can('view_membership_manager')) && ($data > 0) && (isset($GLOBALS['member_area'.$data.'_data'])) && (isset($GLOBALS['member_area'.$data.'_data']['id']))) { $table_td = '<a href="admin.php?page=membership-manager-member-area&amp;id='.$data.'">'.$table_td.'</a>'; } }
else {
$members_areas_list = '';
...
function contact_manager_table_th($tables, $table, $column) {
...
function contact_manager_tablenav_pages($table, $n, $max_paged, $location) {
switch ($table) {
